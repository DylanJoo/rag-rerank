...
def main(args):
    ...
    # Retrieval
    if args.retrieval.model_class == 'bm25':
    ...
    if args.retrieval.model_class == 'contriever':
    ...
    if args.retrieval.model_class == 'splade':
    ...
    # Random reranking 
    import random
    if args.retrieval.seed != 0:
        random.seed(args.retrieval.seed)
        for qid in output_run:
            docids = [docid for docid in output_run[qid]]
            random.shuffle(docids)
            output_run[qid] = {docid: 1/(i+1) for i, docid in enumerate(docids)}
    ...
    if args.generation is not None:
        ...
        # metrics = ['mean_coverage', 'mean_density', 'Recall', 'MAP', 'MAP@100', 'nDCG', 'nDCG@100', 'alpha_nDCG']
        metrics = ['mean_coverage', 'mean_density', 'Recall', 'MAP', 'nDCG', 'alpha_nDCG']
        values = [str(output_rac_eval[m]) for m in metrics]
