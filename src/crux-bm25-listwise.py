...
def main(args):
    ...
    if args.reranking is not None:
        ...
        output_run = rerank(
            ...
            corpus=corpus,
            runs=output_run,
            model_path=args.reranking.model_name_or_path,
            top_k=args.reranking.top_k,
            num_passes=args.reranking.num_passes,
            batch_size=args.reranking.batch_size,
            max_length=args.reranking.max_length,
            prompt_mode='rank_GPT',
            cpntext_size=4096, # this typo
            variable_passages=True,
            use_logits=args.reranking.use_logits,
            use_alpha=True,
            vllm_batched=True,
    ...
    if judgements:
        ...
        output_eval = rac_evaluate(
            ...
            rac_data=output_rac,
            n_questions=15 if 'testb' in args.topic_file else 10,
            threshold=3,
            runs=output_run,
        )
        print(output_eval)
