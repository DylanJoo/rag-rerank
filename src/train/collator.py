...
class Standard:
    ...
    # for GPT
    src_template = "[{}] {}\n"
    tgt_template = "\nSummarize the above documents based on the user request. Write `unrelated` if context is irrelevant and write `redundant` if the information has been summarized. Request: {}\n\nSummary:"
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for example in features:
            ...
            for i, idx in enumerate(random_idx):
                ...
                src_ += [self.src_template.format(i+1, doc_ctx)]
                tgt_ += f"[{i+1}] {comp_ctx} "
                topics.append(topic)
        ...
        outputs = self.tokenizer(
            [self.tgt_template(tp)+tg for tp, tg in zip(topics, tgt_)],
